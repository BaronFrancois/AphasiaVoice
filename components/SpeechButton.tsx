...
import {
    useButtonPress,
    type PressLifecycleData,
} from "../hooks/useButtonPress";
...
interface SpeechButtonProps {
    ...
    colSpan?: number;
    onClick?: () => void;
    onUse?: (metrics?: PressLifecycleData) => void;
...
const SpeechButton: React.FC<SpeechButtonProps> = ({
    ...
    const buttonPressHandlers = useButtonPress({
        ...
        onPressComplete: (metrics) => {
            if (onUse && metrics.valid) {
                onUse(metrics);
            }
        },
        minPressDuration: 60, // ultra-reactif pour valider quasi immediatement
        moveThreshold: 30,
    ...
    return (
        <div
            data-tile-id={id}
            className={`relative ${
                fullHeight ? "h-full" : "h-full min-h-[140px]"
            } w-full ${className}`}
        >
            <button
                onPointerDown={(e) => {
          ...
          shadow-lg
          ${
              !isEditing && !isPressing
                  ? "active:border-b-0 active:translate-y-[6px] active:shadow-none"
                  : ""
          }
          ${
              isPressing && !isEditing
                  ? "scale-95 brightness-90"
                  : ""
          }
          ${
              isEditing
                  ? "cursor-grab border-dashed border-4 border-white/30"
                  : ""
          }
          ${
              isSelected
                  ? "ring-4 ring-yellow-400 scale-95 opacity-90 z-10"
                  : ""
          }
        `}
            >
                {Icon && (
                    <Icon
                        size={64}
                        className={`mb-4 ${
                            isEditing ? "opacity-50" : "opacity-90"
                        } pointer-events-none transition-opacity filter drop-shadow-md`}
                        strokeWidth={2.5}
                    />
                )}

                <span
                    className={`text-3xl sm:text-4xl font-bold uppercase tracking-wider text-center px-4 leading-tight pointer-events-none select-none drop-shadow-sm ${
